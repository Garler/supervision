...
```
Ответ: Нет, в job вызван метод child.cancel() и задержка меньше чем в child.
...
```
Ответ: Да, т.к. функция coroutineScope позволяет перехватывать исключения и предоставляет их в виде Exception.
...
```
Ответ: Да, supervisorScope позволяет перехватывать исключения и не влиять на другие карутины (но в данном примере их нет).
